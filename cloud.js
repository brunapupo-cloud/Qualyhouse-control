(() => {
  ...
  function injectAuthUI() {
    ...
    style.textContent = `
      ...
      .qh-loading-card{text-align:center}
      .qh-loading-text{font-size:18px;font-weight:700;margin-top:16px;color:#374151}
    `;
    ...
    auth.innerHTML = `
      <div id="qh-loading-card" class="qh-auth-card qh-loading-card">
        <div class="qh-auth-brand">QUALYHOUSE CONTROL</div>
        <div class="qh-loading-text">Carregando seus dados...</div>
      </div>
  ...
  function showLoading() {
    ...
    document.getElementById('qh-loading-card')?.classList.remove('hide');
    document.getElementById('qh-login-card')?.classList.add('hide');
  }

  function showLogin(message = '') {
    if (appEl) appEl.classList.add('hide');
    document.getElementById('qh-auth')?.classList.remove('hide');
    document.getElementById('qh-loading-card')?.classList.add('hide');
    document.getElementById('qh-login-card')?.classList.remove('hide');
  ...
  async function loadCloudData() {
    ...
    const unitById={};
    (unitsRes.data||[]).forEach(row=>{const prop=propertyById[row.property_id];if(!prop)return;const key=`${prop.code}-${row.unit_number}`;const base=fresh.units[key]||{property:prop.name,unit:row.unit_number,type:'Apartamento',bedrooms:'',status:'livre',tenant:'',phone:'',email:'',rent:'',dueDay:'',entryDate:'',exitDate:'',contractStatus:'',notes:'',energyUC:'',payments:[],maintenance:[]};fresh.units[key]={...base,_id:row.id,_propertyId:row.property_id,property:prop.name,unit:row.unit_number,type:row.type||base.type||'Apartamento',bedrooms:row.bedrooms==null?(base.bedrooms||''):row.bedrooms,status:row.status||'livre',tenant:row.tenant_name||'',phone:row.phone||'',email:row.email||'',rent:row.monthly_rent==null?'':String(row.monthly_rent),dueDay:row.due_day==null?'':String(row.due_day),entryDate:row.entry_date||'',exitDate:row.exit_date||'',contractStatus:row.contract_status||'',notes:row.notes||'',energyUC:row.energy_uc||'',payments:[],maintenance:base.maintenance||[]};unitById[row.id]=fresh.units[key];});
  ...
  saveUnit=async function(){if(!currentUnitKey||!authUser)return false;const u=db.units[currentUnitKey];if(!u?._id){alert('Unidade não encontrada na nuvem.');return false;}u.status=document.getElementById('u-status').value;u.tenant=document.getElementById('u-tenant').value.trim();u.phone=document.getElementById('u-phone').value.trim();u.email=document.getElementById('u-email').value.trim();u.rent=document.getElementById('u-rent').value.replace(',','.');u.dueDay=document.getElementById('u-due').value.trim();u.entryDate=document.getElementById('u-entry').value;u.exitDate=document.getElementById('u-exit').value;u.contractStatus=document.getElementById('u-contract').value.trim();const en=document.getElementById('u-energy');if(en)u.energyUC=en.value.trim();u.notes=document.getElementById('u-notes').value;const{error}=await sb.from('units').update({type:u.type||'Apartamento',bedrooms:u.bedrooms===''?null:Number(u.bedrooms),status:u.status,tenant_name:u.tenant||null,phone:u.phone||null,email:u.email||null,monthly_rent:u.rent===''?null:Number(u.rent),due_day:u.dueDay===''?null:Number(u.dueDay),entry_date:u.entryDate||null,exit_date:u.exitDate||null,contract_status:u.contractStatus||null,energy_uc:u.energyUC||null,notes:u.notes||null,updated_at:new Date().toISOString()}).eq('id',u._id).eq('owner_id',authUser.id);if(error){console.error(error);alert('Não foi possível salvar na nuvem. Tente novamente.');return false;}save();if(currentUnitKey.startsWith('villa'))setFloor(currentFloor);toast('Alterações salvas na nuvem');return true;};
  savePayment=async function(){if(!authUser)return;const u=db.units[currentUnitKey];if(!u?._id)return alert('Unidade não encontrada na nuvem.');const payload={owner_id:authUser.id,property_id:u._propertyId||null,unit_id:u._id,avulso_id:null,competence:document.getElementById('p-comp').value||null,payment_date:document.getElementById('p-date').value,method:document.getElementById('p-method').value||null,amount:Number((document.getElementById('p-value').value||'0').replace(',','.')),notes:document.getElementById('p-note').value.trim()||null};const{data,error}=await sb.from('payments').insert(payload).select().single();if(error){console.error(error);return alert('Não foi possível salvar o pagamento na nuvem.');}u.payments=u.payments||[];u.payments.push({_id:data.id,competence:data.competence||'',date:data.payment_date||'',value:Number(data.amount||0),method:data.method||'',note:data.notes||''});save();document.getElementById('payment-dialog').close();openUnit(currentUnitKey);toast('Pagamento salvo na nuvem');};
  saveAvulsoPayment=async function(i){if(!authUser)return;const a=db.avulsos[i];if(!a||!a._id)return alert('Locação não encontrada na nuvem.');const amount=Number((document.getElementById('ap-value').value||'0').replace(',','.'));if(!amount)return alert('Informe o valor recebido.');const payload={owner_id:authUser.id,property_id:null,unit_id:null,avulso_id:a._id,competence:document.getElementById('ap-comp').value||null,payment_date:document.getElementById('ap-date').value,method:document.getElementById('ap-method').value||null,amount:amount,notes:document.getElementById('ap-note').value.trim()||null};const res=await sb.from('payments').insert(payload).select().single();if(res.error){console.error(res.error);return alert('Não foi possível salvar o pagamento na nuvem.');}const data=res.data;a.payments=a.payments||[];a.payments.push({_id:data.id,competence:data.competence||'',date:data.payment_date||'',value:Number(data.amount||0),method:data.method||'',note:data.notes||''});save();document.getElementById('payment-dialog').close();editAvulso(i);toast('Pagamento salvo na nuvem');};
  async function updatePaymentRow(id){const amount=Number((document.getElementById('ep-value').value||'0').replace(',','.'));if(!amount){alert('Informe o valor recebido.');return null;}const payload={competence:document.getElementById('ep-comp').value||null,payment_date:document.getElementById('ep-date').value,amount:amount,method:document.getElementById('ep-method').value||null,notes:document.getElementById('ep-note').value.trim()||null};const res=await sb.from('payments').update(payload).eq('id',id).eq('owner_id',authUser.id).select().single();if(res.error){console.error(res.error);alert('Não foi possível atualizar o pagamento na nuvem.');return null;}return res.data;}
  updatePayment=async function(key,id){if(!authUser)return;const u=db.units[key],p=(u?.payments||[]).find(x=>x._id===id);if(!p)return;const data=await updatePaymentRow(id);if(!data)return;Object.assign(p,{competence:data.competence||'',date:data.payment_date||'',value:Number(data.amount||0),method:data.method||'',note:data.notes||''});save();document.getElementById('payment-dialog').close();openUnit(key);toast('Pagamento atualizado na nuvem');};
  updateAvulsoPayment=async function(i,id){if(!authUser)return;const a=db.avulsos[i],p=(a?.payments||[]).find(x=>x._id===id);if(!p)return;const data=await updatePaymentRow(id);if(!data)return;Object.assign(p,{competence:data.competence||'',date:data.payment_date||'',value:Number(data.amount||0),method:data.method||'',note:data.notes||''});save();document.getElementById('payment-dialog').close();editAvulso(i);toast('Pagamento atualizado na nuvem');};
  saveExpense=async function(){if(!authUser)return;const description=document.getElementById('exp-description').value.trim();const value=Number((document.getElementById('exp-value').value||'0').replace(',','.'));if(!description||!value){alert('Preencha a descrição e o valor da despesa.');return;}const propertyName=document.getElementById('exp-property').value;const payload={owner_id:authUser.id,property_id:propertyIdFromName(propertyName),unit_id:null,category:document.getElementById('exp-category').value||null,description,expense_date:document.getElementById('exp-date').value,amount:value,notes:document.getElementById('exp-notes').value||null};const{data,error}=await sb.from('expenses').insert(payload).select().single();if(error){console.error(error);return alert('Não foi possível salvar a despesa na nuvem.');}db.expenses=db.expenses||[];db.expenses.push({_id:data.id,property:propertyName,category:data.category||'Outros',description:data.description||'',value:Number(data.amount||0),date:data.expense_date||'',notes:data.notes||'',_propertyId:data.property_id||null,_unitId:data.unit_id||null});save();document.getElementById('expense-dialog').close();toast('Despesa salva na nuvem');};
  saveVillaInfo=async function(){if(!authUser||!propertyRowsByCode.villa)return;const energyUC=document.getElementById('villa-energy').value.trim();const waterUC=document.getElementById('villa-water').value.trim();const{error}=await sb.from('properties').update({energy_uc:energyUC||null,water_uc:waterUC||null,updated_at:new Date().toISOString()}).eq('id',propertyRowsByCode.villa.id).eq('owner_id',authUser.id);if(error)return alert('Não foi possível salvar as informações na nuvem.');db.properties.villa.energyUC=energyUC;db.properties.villa.waterUC=waterUC;save();toast('Informações salvas na nuvem');};
  saveGirassolInfo=async function(){if(!authUser||!propertyRowsByCode.girassol)return;const waterHydrometer=document.getElementById('gir-water').value.trim();const{error}=await sb.from('properties').update({water_hydrometer:waterHydrometer||null,updated_at:new Date().toISOString()}).eq('id',propertyRowsByCode.girassol.id).eq('owner_id',authUser.id);if(error)return alert('Não foi possível salvar o hidrômetro na nuvem.');db.properties.girassol.waterHydrometer=waterHydrometer;save();toast('Hidrômetro salvo na nuvem');};
